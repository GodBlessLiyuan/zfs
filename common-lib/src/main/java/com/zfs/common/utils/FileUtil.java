...
import com.zfs.common.constant.ModuleConstant;
import com.zfs.common.dto.MakeDTO;
import com.zfs.common.jna.Clibrary;
...
public class FileUtil {
    ...
    private static boolean modifyApkPkg(String xmlPath, MakeDTO dto, String zipPath) throws IOException, InterruptedException {
        ...
        // 修改2处 meta-data
        if (null != dto.getApppkg() && !"".equals(dto.getApppkg())) {
            FileUtil.modifyApkPkg("meta-data", 1, "value", dto.getApppkg(), xmlPath, outXml);
            flag = true;
        }

        if (null != dto.getIsolation() && !"".equals(dto.getIsolation())) {
            FileUtil.modifyApkPkg("meta-data", 2, "value", dto.getIsolation(), xmlPath, outXml);
            flag = true;
        }
    ...
    private static void modifyApkPkg(String application, int index, String name, String value, String inputXml, String outXml) throws IOException, InterruptedException {
        String[] CMD_STR = new String[]{"/bin/sh", "-c", "/data/smarthelper/tools/ameditor a --modify " + application + " -d " + index + " -n " + name + " -t 3 -v "
    ...
    private static String modifyApkSign(String signApkUrl, String tempFilePath, boolean isModXml) throws IOException, InterruptedException {
        ...
        CMD_STR = new String[]{"/bin/sh", "-c", "jarsigner -digestalg SHA1 -sigalg MD5withRSA -verbose "
                + "-keystore /data/smarthelper/tools/godArmor.keystore -storepass 123456 -signedjar " + signApkUrl + " " + tempFilePath + TEMPLATE_APK_NAME + " godArmor.keystore"};
        process = Runtime.getRuntime().exec(CMD_STR);
        process.waitFor();
