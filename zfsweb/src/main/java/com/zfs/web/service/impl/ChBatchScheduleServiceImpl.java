...
public class ChBatchScheduleServiceImpl {
    ...
    public void overLineCount(){
        //先计算结束的
        minLineCount((byte)1,(byte)2);
        //冻结(3)和过期冻结（7）
        minLineCount((byte)3,(byte)7);
        //失效(4)和过期失效（6）
        minLineCount((byte)4,(byte)6);

    }

    private void minLineCount(Byte originStatu,Byte toStatu){
        //联合表查询，创建时间+天数小于现在时刻，从数据库构造时间
        List<Integer> batchIdS=chBatchMapper.getListStatusAndGroup(originStatu);
        if(batchIdS==null||batchIdS.size()==0){
            logger.info("批次信息表没有{}从状态到状态{}的更新",originStatu,toStatu);
            return;
        }
        for(Integer batchId:batchIdS){
            //查询该状态下的值，并且更新该状态下的值
            List<Integer> infoS=batchInfoMapper.queryStatus(batchId,originStatu);
        ...
        logger.info("更新了t_ch_batch表主键为{}的状态:，t_batch_info表的主键为{}的状态：", JSON.toJSONString(batchIdS));
    }
}
